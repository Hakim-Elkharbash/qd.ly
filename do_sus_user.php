...
$user_id=$_GET['id'];
$user_info=mysql_fetch_assoc(mysql_query("SELECT * FROM sys_users_lp WHERE id='$user_id'"));
if ($user_info[suspended]=='yes'){
$ssu='';
}else{
$ssu='yes';
}
$result = mysql_query("UPDATE sys_users_lp SET suspended='$ssu' WHERE id='$user_id'");

if ($result){

echo '<div  style="font-size:20px; font-weight: bold; padding-bottom:20px">
<span class="fa fa-check fa-3x"></span>
لقد تم إيقاف المستخدم بنجاح...
</div> '; 
...
حدث خطأ في عملية الإيقاف. قد تكون هذه البيانات مضافة سابقا...
</div> '; 
