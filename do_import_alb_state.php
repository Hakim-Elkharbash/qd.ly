...
$result = mysql_query("UPDATE qd_orders SET alb_update='$alb_update' WHERE way_bill='$way_bill'");
...
$operation="تغيير حالة الباران - إستراد";
$details=$alb_update;
...
لقد تم تغيير حالة الباران ('.$corr_yes.') طلبية بنجاح (كل الطلبيات) وذلك من أصل ('.$arrayCount_r.') طلبية.
</div> '; 
}elseif ($corr_yes==0){
echo '<span class="fa fa-times fa-3x"></span>  
لم يتم تغيير حالة الباران إي طلبية من أصل ('.$arrayCount_r.') طلبية، السبب: قد تكون هذه الطلبية غير مضافة أصلاَ.
</div> '; 
//echo mysql_error();
}else{
echo '<span class="fa fa-info fa-3x"></span>  
لقد تم تغيير حالة الباران ('.$corr_yes.') طلبية فقط من أصل ('.$arrayCount_r.') طلبية، السبب: قد يكون جزء من هذه الطلبية غير مضافة أصلاَ.
</div> '; 
}
