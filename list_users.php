        ...
        <div class="text-center">
            <div class="panel panel-default col-lg-10 col-lg-offset-1" style="padding-bottom:20px !important">
               <div class="panel-heading" style="background-color: burlywood !important">
                    <h4 style="text-align:right;"> <i class="fa fa-users fa-md"></i> إدارة المستخدمين.. </h4>
                </div>
                <div class="panel-body ">              
                    
<?
$qd_list=mysql_query("SELECT * FROM sys_users_lp");
...
<table class="table table-striped table-hover">
  <thead>
    <tr>
      ...
      <th style="text-align:center; " class="success">الاسم</th>
      <th style="text-align:center; " class="success">رقم الهاتف</th>
      <th style="text-align:center; " class="success">نوع الحساب</th>
...
while($rows=mysql_fetch_array($qd_list)){
    echo '<tr>
      ...
      <td style="text-align:center;vertical-align: middle">'.$rows[fullname].'</td>
      <td style="text-align:center;vertical-align: middle">'.$rows[phone].'</td>
      <td style="text-align:center;vertical-align: middle">';
    
    if ($rows[occupation]=='qd_employee'){
    echo "موظف بـ QD";    
    }elseif ($rows[occupation]=='cl_supervisor'){
    echo "مشرف بـ CL";    
    }elseif ($rows[occupation]=='cl_distributor'){
    echo "مندوب بـ CL";    
    }else{
    echo "أخرى";    
    }
    ...
    echo '</td>
      <td style="text-align:center;vertical-align: middle">
        ...
        <a href="edit_user.php?id='.$rows[id].'" class="btn btn-info  btn-md navbar-btn">
                    تعديل&nbsp;
                <span class="fa fa-pencil-square-o"></span>
        </a>&nbsp;';
        
    echo '<a href="user_permission.php?id='.$rows[id].'" class="btn btn-success  btn-md navbar-btn">
                صلاحيات&nbsp;
                <span class="fa fa-pencil-square-o"></span>
        </a>&nbsp;';
    
    $user_info=mysql_fetch_assoc(mysql_query("SELECT * FROM sys_users_lp WHERE id='$rows[id]'"));
    if ($user_info[suspended]=='yes'){
        echo '<a id="model-1" class="btn btn-warning btn-md navbar-btn" onclick="msg_info_sus('.$rows[id].')" href="javascript:void(0);">
        تفعيل&nbsp;  <span class="fa fa-trash-o"></span>  </a>&nbsp;';
    }else{
    echo '<a id="model-1" class="btn btn-warning btn-md navbar-btn" onclick="msg_info_sus('.$rows[id].')" href="javascript:void(0);">
        إيقاف&nbsp;  <span class="fa fa-trash-o"></span>  </a>&nbsp;';
    }
...
?>                
    ...
    <script>
            ...
            bootbox.dialog({
              ...
              buttons: {
                ...
                danger: {
                  ...
                  callback: function () {
                      location.href = "do_del_user.php?id="+xx;
                  }
                }
              }
            });
        ...
        function msg_info_sus(xx){
    
            bootbox.dialog({
              message: "<span style='font-size:16px; font-weight: bold'>هل ترغب بالفعل في إيقاف / تفعيل هذا المستخدم </span>",
              title: "لوحة التحكم",
              buttons: {
                success: {
                  label: "إلغاء الامر",
                ...
                danger: {
                  label: "إيقاف / تفعيل مستخدم",
                  className: "btn-danger",
                  callback: function () {
                      location.href = "do_sus_user.php?id="+xx;
                  }
                }
              }
            });
