...
<?
$city_id=$_GET['id'];
$qd_list=mysql_fetch_array(mysql_query("SELECT * FROM qd_city WHERE id='$city_id'"));
 ?>
        ...
        <div class="text-center">
            <div class="panel panel-default col-lg-6 col-lg-offset-3" style="padding-bottom:20px !important">
               <div class="panel-heading" style="background-color: burlywood !important">
                    <h4 style="text-align:right;"> <i class="fa fa-map-marker fa-md"></i> تعديل مدينة.. </h4>
                </div>
                <div class="panel-body ">             
                    <form action="do_edit_city.php" method="post" class="form-horizontal" id="qd-form" name="qd-form-nm">
                      ...
                      <div class="form-group">
                           ...
                           <select name="arlist" class="form-control input-lg">
                               ...
                               $qd_list_ar=mysql_query("SELECT * FROM qd_areas");
                                while($rows=mysql_fetch_array($qd_list_ar)){
                                    if ($qd_list[area_id]==$rows[id]){
                                        echo "<option selected value=".$rows[id].">".$rows[area_ar]." (".$rows[area_en].")</option>";
                                    }else{
                                        echo "<option value=".$rows[id].">".$rows[area_ar]." (".$rows[area_en].")</option>";
                                    }
                                }
                                ?>
                            </select>
                        ...
                        <div class="form-group">
                        <div class="col-sm-12">
                            <label for="inputName1" class="pull-right control-label fields-name"> اسم المدينة - انجليزي </label>
                            <input type="text" name="ename" class="form-control input-lg" value="<? echo $qd_list[city_en]; ?>" id="inputName1"  placeholder="اسم المدينة - انجليزي" style="text-align: left; direction: ltr">
                        </div>
                      </div>
                        
                      <div class="form-group">
                        <div class="col-sm-12">
                            <label for="inputName2" class="pull-right control-label fields-name"> اسم المدينة - عربي </label>
                            <input type="text" name="aname" class="form-control input-lg" value="<? echo $qd_list[city_ar]; ?>" id="inputName2"  placeholder="اسم المدينة - عربي" >
                      ...
                      <div class="form-group">
                        <div class="col-sm-12 pull-right">
                          <button name="login_utton" type="submit" class="btn btn-primary btn-lg"> تعديل المدينة </button>
		...
		var validator = $("#qd-form").bootstrapValidator({
			...
			fields : {
				ename : {
					validators: {
						notEmpty : {
							message : "يجب إدخال اسم المدينة بالانجليزي"
						}
					}
				}, 
				aname : {
					validators: {
						notEmpty : {
							message : "يجب إدخال اسم المدينة بالعربي"
						}
					}
				},
                arlist : {
					validators: {
						notEmpty : {
							message : "يجب أختيار اسم المنطقة"
						}
					}
				}
			}
