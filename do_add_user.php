...
$result = mysql_query("INSERT INTO sys_users_lp(username,password,fullname,phone,email,access_services,occupation,user_no) VALUES ('$email','$e_pass','$fullname','$phone','$email','$access_ser','$userlist','$user_no')");
if ($result){

$uu_id=mysql_insert_id();
echo '<div  style="font-size:20px; font-weight: bold; padding-bottom:20px">
<span class="fa fa-check fa-3x"></span>
لقد تم تكوين الحساب بنجاح...
</div> '; 

//--------------------- give premissions
if ($userlist=='cl_supervisor'){
$services="9 15 21";

$store_pre=mysql_query("SELECT * FROM qd_store");
$store='';   
while($rows=mysql_fetch_array($store_pre)){
$store=$store.$rows[id].' ';    
}  
    
$state_pre=mysql_query("SELECT * FROM qd_state");
$state='';   
while($rows=mysql_fetch_array($state_pre)){
$state=$state.$rows[id].' '; 
}  
    
$areas_pre=mysql_query("SELECT * FROM qd_areas");
$areas='';   
while($rows=mysql_fetch_array($areas_pre)){
$areas=$areas.$rows[id].' '; 
}  
  
$city_pre=mysql_query("SELECT * FROM qd_city");
$city='';   
while($rows=mysql_fetch_array($city_pre)){
$city=$city.$rows[id].' '; 
}  
    
$result = mysql_query("UPDATE sys_users_lp SET access_stores='$store',access_services='$services',access_status='$state',access_areas='$areas',access_city='$city' WHERE id='$uu_id'");
}
//-------------------------------------
...
//-------------------- إرسال بريد الكتروني
$to=$email;
$subject="التسجيل بموقع QDelivery";
...
<p class="style1"> لقد تم تسجيلكم في موقع QDelivery نرجوا أستلام بيانات الدخول الخاصة بكم. </p>
<p class="style3">~~~~~~~~~~~~~~</p>
...
echo '
<a href="user_permission.php?id='.$uu_id.'" class="btn btn-success btn-md navbar-btn btn-lg">
                                    <span class="fa fa-tasks"></span>
...
echo '<div  style="font-size:20px; font-weight: bold; padding-bottom:20px">
<span class="fa fa-times fa-3x"></span>
حدث خطأ في عملية تكوين الحساب، قد تكون هذه البيانات مضافة سابقا...
</div> '; 
