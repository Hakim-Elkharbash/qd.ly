...
$state_info=mysql_fetch_array(mysql_query("SELECT * FROM qd_state WHERE state_en='$state_to'"));
$ar_state=$state_info[state_ar];

$itm_id=$_POST['itm_id'];
$crr=0;
foreach ($itm_id as &$way_bill) {
$result = mysql_query("UPDATE qd_orders SET store='$change_to',shipment_state_en='$state_to',shipment_state_ar='$ar_state' WHERE way_bill='$way_bill'");
if ($result){
$crr++;
...
$order_id=$way_bill;
$data_time=date('d/m/Y H:i:s');;
$operation="تغيير الحالة أثناء التحريك بين المخازن";
$details=$state_to;
$user_info=$user_infos[fullname];
$result = mysql_query("INSERT INTO qd_log(order_id,data_time,operation,details,user_info) VALUES ('$order_id','$data_time','$operation','$details','$user_info')");
//--------------- end log
...
';
    ...
    echo '  
<a href="list_move_orders.php" class="btn btn-warning btn-md navbar-btn btn-lg">
...
يجب ان تقوم بتحديد واحد على الاقل حتى تتم العملية المطلوبة...
 ...
 echo '
<a href="list_move_orders.php" class="btn btn-info btn-md navbar-btn btn-lg">
                                    <span class="fa fa-chevron-left fa-rotate-180"></span>
