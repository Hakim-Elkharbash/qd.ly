        ...
        <div class="text-center">
            <div class="panel panel-default col-lg-10 col-lg-offset-1" style="padding-bottom:20px !important">
               <div class="panel-heading" style="background-color: burlywood !important">
                   ...
                   <h4 style="text-align:right;"> <i class="fa fa-arrow-right fa-md"></i> تحريك البضائع بين المخازن.. </h4>
...
<form action="do_orders_move.php" method="post"  id="qd-form2" name="qd-form-nm2">
    ...
    echo '
       <div class="well"  style="height: 90px">            
            <div class="col-sm-5">
                            <select name="change_to" class="form-control input-lg">'; 
                               ...
                               //$permissions_st= explode(" ",$qd_list1[access_stores]);
                               $qd_list1_st=mysql_query("SELECT * FROM qd_store");
                                while($rows=mysql_fetch_array($qd_list1_st)){
                                    //if (in_array($rows[id], $permissions_st)){ 
                                        echo "<option value='".$rows[store_en]."'>".$rows[store_ar]." (".$rows[store_en].")</option>";
                                    //}
                                }
             ...
             <div class="col-sm-4">
                            <select name="state_to" class="form-control input-lg">';
                               $permissions_stt= explode(" ",$qd_list1[access_status]);
                               $qd_list_stt=mysql_query("SELECT * FROM qd_state");
                                while($rows=mysql_fetch_array($qd_list_stt)){
                                  if (in_array($rows[id], $permissions_stt)){ 
                                        echo "<option value='".$rows[state_en]."'>".$rows[state_ar]." (".$rows[state_en].")</option>";
            ...
            <div class="col-sm-3">
                    <button name="login_utton" type="submit" class="btn btn-primary btn-lg"> تحريك الى المخزن الجديد </button>
     ...
     echo '
       <div class="well"  style="height: 90px">            
            ...
            <div class="col-sm-4">
                            <select name="state_to" class="form-control input-lg" disabled="disabled">';
                               $permissions_stt= explode(" ",$qd_list1[access_status]);
                               $qd_list_stt=mysql_query("SELECT * FROM qd_state");
                                while($rows=mysql_fetch_array($qd_list_stt)){
                                  if (in_array($rows[id], $permissions_stt)){ 
            ...
            <div class="col-sm-3">
                    <button name="login_utton" type="submit" class="btn btn-primary btn-lg" disabled="disabled"> تحريك الى المخزن الجديد </button>
		...
		var validator = $("#qd-form2").bootstrapValidator({
			...
			fields : {
                change_to : {
					validators: {
						notEmpty : {
							message : "يجب تحديد المخزن الذي ترغب في نقل البضائع إليه"
						}
					}
				}
			}
