        ...
        <div class="text-center">
            <div class="panel panel-default col-lg-6 col-lg-offset-3" style="padding-bottom:20px !important">
               <div class="panel-heading" style="background-color: burlywood !important">
                    <h4 style="text-align:right;"> <i class="fa fa-pencil-square-o fa-md"></i>    تعديل طلبية (إستراد).. </h4>
                </div>
                <div class="panel-body ">             
                    <form action="do_import_update_order.php" method="post" class="form-horizontal" id="qd-form" name="qd-form-nm" enctype="multipart/form-data">
                      
                      <!-- 
                        <div class="form-group">
                        <div class="col-sm-12">
                            <label for="inputName1" class="pull-right control-label fields-name"> تحديد رقم الطلبية </label>
                            <?/*
                                $qd_list_st=mysql_query("SELECT * FROM qd_orders");
                                $order_st[]="";
                                while($rows=mysql_fetch_array($qd_list_st)){
                                    if (!in_array($rows[order_no], $order_st)){ 
                                       $order_st[]=$rows[order_no];
                                    }
                                }
                              ?>
                            <select name="ordlist" class="form-control input-lg">
                              <?
                                unset($order_st[0]);
                                arsort($order_st);
//$num=count($order_st);
//$num=$num-1;
                                foreach ($order_st as $key => $val) {
                    ...
                    -->
                        <div class="form-group">
                        <div class="col-sm-12">
                            <label for="inputName2" class="pull-right control-label fields-name">  رقم الطلبية الجديد </label>
                            <input type="text" name="oname" class="form-control input-lg" id="inputName2"  placeholder="تغيير رقم الطلبية" style="text-align: left; direction: ltr">
                            <label for="inputName6" class="pull-right control-label fields-name"> تترك فارغة في حال لا ترغب في تغيير رقم الطلبية. </label>
                        ...
                        <div class="form-group">
                        <div class="col-sm-12">
                            <label for="inputName3" class="pull-right control-label fields-name">  تحديد المخزن الجديد </label>
                            <select name="storlist" class="form-control input-lg">
                              <?
                                echo "<option value='default'> الحالة الإفتراضية (الموجودة في قاعدة البيانات) </option>";
                         ...
                         <div class="form-group">
                        <div class="col-sm-12">
                            <label for="inputName4" class="pull-right control-label fields-name"> تحديد الحالة الجديدة  </label>
                            <select name="stlist" class="form-control input-lg">
                              <?
                                //echo "<option value='non_change'> بدون تغيير </option>";
