...
لقد تم تغيير حالة ('.$corr_yes.') طلبية بنجاح (كل الطلبيات) وذلك من أصل ('.$arrayCount_r.') طلبية.
</div> '; 
}elseif ($corr_yes==0){
echo '<span class="fa fa-times fa-3x"></span>  
لم يتم تغيير حالة إي طلبية من أصل ('.$arrayCount_r.') طلبية، السبب: قد تكون هذه الطلبية غير مضافة أصلاَ.
</div> '; 
//echo mysql_error();
}else{
echo '<span class="fa fa-info fa-3x"></span>  
لقد تم تغيير حالة ('.$corr_yes.') طلبية فقط من أصل ('.$arrayCount_r.') طلبية، السبب: قد يكون جزء من هذه الطلبية غير مضافة أصلاَ.
</div> '; 
}
...
<a href="list_report_xls.php" class="btn btn-success btn-md navbar-btn btn-lg">
                    ...
                    &nbsp;  تحميل كملف Excel (العناصر غير المعدلة)
                </a> 
';
}
...
<p class="style1"> لقد قام ('.$user_info.') بتغيير ('.$corr_yes.') عنصر (إستراداً) وذلك بتاريخ ('.$dat_tim.'). </p>
<p class="style3">~~~~~~~~~~~~~~</p>
